...
当前为测试签名版本，可用于授权测试或内部验证，但不能表述为生产签名版本。
...
注意：当前 KMDF 包使用测试签名，仅用于授权测试或内部验证。创建或更新
KMDF 端口时，程序可能会将随包 `VComTunnel.Serial.cer` 测试证书写入本机证书
存储区、安装或更新驱动，并要求重启。

GUI 的“安装依赖”不会自动安装 KMDF 驱动。只有用户主动选择 `kmdf` 映射，并创建
或更新该 KMDF 端口时，才会显示测试签名驱动确认提示。Windows 可能需要
